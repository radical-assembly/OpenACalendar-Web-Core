...
namespace org\openacalendar\radicalassembly\repositories\builders;
 ...
 */
class EventRepositoryBuilder extends \repositories\builders\BaseRepositoryBuilder {
    ...
    public function setSite($site) {
        $this->site = ($site instanceof SiteModel) ? array($site) : $site;
    }
    ...
    public function setGroup($group) {
        $this->group = ($group instanceof GroupModel) ? array($group) : $group;
    }
    ...
    public function setCountry($country) {
        $this->country = ($country instanceof CountryModel) ? array($country) : $country;
    }
    ...
    public function setArea($area) {
        $this->area = ($area instanceof AreaModel) ? array($area) : $area;
    }
    ...
    public function setVenue($venue) {
        $this->venue = ($venue instanceof VenueModel) ? array($venue) : $venue;
    }
    ...
    public function setImportURL($importURL) {
        $this->importURL = ($importURL instanceof ImportURLModel) ? array($importURL) : $importURL;
    }
	...
	public function setCuratedList($curatedList, $curatedListInformation = false) {
		$this->curatedList = ($curatedList instanceof CuratedListModel) ? array($curatedList) : $curatedList;
	...
	public function setTag($tag) {
		$this->tag = ($tag instanceof TagModel) ? array($tag) : $tag;
	}
    ...
    public function setEditedByUser($editedByUser) {
        $this->editedByUser = ($editedByUser instanceof UserAccountModel) ? array($editedByUser) : $editedByUser;
    }
    ...
    protected function catSQL($prefix, $objarr) {
        $arg = array();
        for ($i = 0; $i < count($this->site); $i++) {
            $name = $prefix."_".(string)$i;
            $arg[] = ":".$name;
            $this->parms[$name] = $objarr[$i]->getId();
        }
        return " (" . implode(", ", $arg) . ") ";
    }
    ...
    protected function build() {
        ...
        if ($this->site) {
            $this->where[] = " event_information.site_id IN ".$this->catSQL("site_id", $this->site);
        }
        ...
        if ($this->group) {
            ...
            $this->joins[] =  " JOIN event_in_group AS event_in_group_select ON event_in_group_select.event_id = event_information.id ".
                    "AND event_in_group_select.removed_at IS NULL AND ";
            $this->joins[] = "event_in_group_select.group_id IN ".$this->catSQL("group_id", $this->group);
        }

        if ($this->country) {
            $this->where[] = "event_information.country_id IN ".$this->catSQL("country_id", $this->country);
        }
        ...
        if ($this->venue) {
            $this->where[] = "event_information.venue_id IN ".$this->catSQL("venue_id", $this->venue);
        }
        ...
        if ($this->importURL) {
            ...
            $this->where[] = "(" .
                                "imported_event.import_url_id IN ".$this->catSQL("import_url_id", $this->importURL) .
                                " OR " .
                                "event_information.import_url_id IN ".$this->catSQL("import_url_id", $this->importURL) .
                                ")";
        }

        if ($this->importedEvent) {
            $this->joins[] = " JOIN imported_event_is_event ON imported_event_is_event.event_id = event_information.id AND ";
            $this->joins[] = "imported_event_is_event.imported_event_id IN ".$this->catSQL("imported_event_id", $this->importedEvent);
        }
        ...
        if ($this->curatedList) {
            ...
            $this->joins[] = "event_in_curated_list.curated_list_id IN ".$this->catSQL("curated_list", $this->curatedList);
        ...
        if ($this->userAccount) {
            ...
            $this->joins[] = "  LEFT JOIN user_at_event_information ON user_at_event_information.event_id = event_information.id ".
                    "AND ";
            $inString = $this->catSQL("user_account_id", $this->userAccount);
            $this->joins[] = "user_at_event_information.user_account_id IN ".$inString;
            ...
            if (!$this->userAccountIncludeAll) {
                ...
                if ($this->userAccountIncludeWatching) {
                    $w[] = "  event_information.id IN (SELECT event_information.id FROM event_information ".
                        ...
                        "AND user_watches_site_information.user_account_id IN ".$inString." AND user_watches_site_information.is_watching='1' ".
                        // group
                        ...
                        " LEFT JOIN user_watches_group_information ON user_watches_group_information.group_id = event_in_group.group_id ".
                            "AND user_watches_group_information.user_account_id IN ".$inString." AND user_watches_group_information.is_watching='1' ".
                        // area
                        ...
                        ") ".
                        "AND user_watches_area_information.user_account_id = ".$inString." AND user_watches_area_information.is_watching='1' ".
        ...
        if ($this->tag) {
            $this->joins[] = "  JOIN event_has_tag ON event_has_tag.event_id = event_information.id AND  event_has_tag.tag_id IN ".
            $this->catSQL("tag_id", $this->tag)." AND event_has_tag.removed_at IS NULL";
        }
        ...
        if ($this->editedByUser) {
            $this->where[] = " event_information.id IN (SELECT event_id FROM event_history WHERE user_account_id IN ".$this->catSQL("editedByUser", $this->editedByUser).") ";
        }
    }
